...
ParseResult FIRStmtParser::parsePrintf() {
  ...
  if (version < FIRVersion(5, 0, 0)) {
    formatStrUnescaped = builder.getStringAttr(FIRToken::getStringValue(formatString));
    operands.append(specOperands.begin(), specOperands.end());
  } else if (failed(circt::firrtl::parseFormatString(
                 builder, translateLocation(formatStringLoc), formatString,
                 specOperands, formatStrUnescaped, operands)))
    return failure();
